# 05: Italian arc variant B (FRA/ENG)

Status: resolved
Type: task
Blocked by: none

**What to build:** the second target variant for the existing Italian arc. Variant A stays (YUG, GRE); variant B is (FRA, ENG), rolled 50/50 at pick. Crises, peak ultimatums, derail and telemetry work identically on B.

- [ ] Pinned observer with forced variant B shows `sc_pick italian_expansion variant=b` and only FRA/ENG in `sc_seed`/`sc_target`.
