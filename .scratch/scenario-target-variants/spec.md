...
- **Scope of this spec**: full catalog (all 28 arcs), delivered as vertical-slice tickets under `.scratch/scenario-target-variants/issues/` (see `01`-`30`). Ticket 01 (target-array engine) is the single structural seam and gates every other ticket; tickets 02-09 add B variants to existing arcs; 10-23 add new historical arcs; 24-30 add flip/imperial arcs and the generic flip gate (ticket 26). All 30 tickets are implemented in `ad10adf` ("Add scenario director with 28 arcs, focus-path docs, and .scratch script layout"), which ships all 28 arcs, the A/B variant roll, and `$sandbox_check_flip_gate*`; every ticket is marked `resolved`. Remaining open work is the observer-session acceptance run, not code.

**Status: implemented** (Sep 2026). The tickets were authored `ready-for-agent` and implemented in bulk rather than ticket-by-ticket, so their statuses lagged the code; they were normalized to `resolved` against a code audit (28 arcs in `sandbox_set_targets`, 28 event files, variants matching the catalog, flip macros present in `common/macros.hml`).
