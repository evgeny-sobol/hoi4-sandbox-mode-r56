...
`scripts/generate_focus_includes.py` rebuilds every Rt56 focus include. It keeps the first-pass splices and then, for IDs that still exist in Rt56, copies the matching `_sandbox` extras (party shares, Honor/rival `available`, Tyranny completion, MIC, `$crossroad_modifier`, CW-root weights). Heuristics fill the rest:
